...
if [ ! -f "timescaledb/iot-data.gz" ]; then
    time $gen_data --use-case="iot" --seed=123 --scale=4000 \
...
if [ ! -f "timescaledb/iot-queries.gz" ]; then
    time $gen_queries --use-case="iot" --seed=123 --scale=4000 \
        --timestamp-start="2016-01-01T00:00:00Z" \
...
time $tsbs_load load timescaledb --config=./iot-config.yaml | tee load.log

# cleanup
docker compose down
docker volume rm benchmark_timescaledb-volume